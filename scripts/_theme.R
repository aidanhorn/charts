...
theme_dark_chart <- function(base_size = 12) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      ...
      plot.title = element_text(colour = CHART_TEXT, face = "bold", hjust = 0),
      plot.subtitle = element_text(colour = CHART_TEXT_DIM, hjust = 0),
      plot.caption = element_text(colour = CHART_TEXT_DIM, size = 8),
